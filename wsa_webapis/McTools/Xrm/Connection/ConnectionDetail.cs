...
namespace McTools.Xrm.Connection
{
    ...
    public class ConnectionDetail : IComparable, ICloneable
    {
        ...
        public CrmServiceClient GetCrmServiceClient(bool forceNewService = false)
        {
            ...
            {
                var temp = HttpContext.Current.Server.MapPath("~/Resources");
                var path = Path.Combine(temp, ConnectionId.Value.ToString("B"));
                if (!File.Exists(path))
                    path = Path.Combine(Path.GetTempPath(), ConnectionId.Value.ToString("B"));
